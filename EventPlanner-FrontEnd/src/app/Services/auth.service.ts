...
export class AuthService {
  // Backend base URL (matches backend README)
  ...
  // Keep a small demo helper to simulate login in places that call `loginAs()` in the app
  loginAs(userId: string) {
    const fake: UserPayload = { id: userId, name: userId, email: `${userId}@example.com`, role: 'attendee' };
    localStorage.setItem(this.userKey, JSON.stringify(fake));
    this.currentUserProfile$.next(fake);
    this.currentUserId$.next(String(userId));
  }
